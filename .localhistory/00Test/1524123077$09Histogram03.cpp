...
int main(int argc, char** argv)
{
	...
	MatND histogramB, histogramG, histogramR , histogramBGR ;
	...
	Mat histImageBGR(hist_h, hist_w, CV_32SC3, Scalar(0, 0, 0));
	normalize(histogramR, histogramR, 0, histImageR.rows, NORM_MINMAX, -1, Mat());
	//normalize(histImageBGR, histImageBGR, 0, histImageBGR.rows, NORM_MINMAX, -1, Mat());
	...
	for (int i = 1; i < number_bins; i++)
	{
		...
		line(histogramBGR, Point(bin_w*(i - 1), hist_h - cvRound(histogramB.at<float>(i - 1))),
			Point(bin_w*(i), hist_h - cvRound(histogramB.at<float>(i))),
			Scalar(255, 0, 0), 2, 8, 0);
		line(histogramBGR, Point(bin_w*(i - 1), hist_h - cvRound(histogramG.at<float>(i - 1))),
			Point(bin_w*(i), hist_h - cvRound(histogramG.at<float>(i))),
			Scalar(0, 255, 0), 2, 8, 0);

		line(histogramBGR, Point(bin_w*(i - 1), hist_h - cvRound(histogramR.at<float>(i - 1))),
			Point(bin_w*(i), hist_h - cvRound(histogramR.at<float>(i))),
			Scalar(0, 0, 255), 2, 8, 0);
	...
	namedWindow("HistogramBGR", CV_WINDOW_AUTOSIZE);
	...
	imshow("HistogramBGR", histogramBGR);	
